...
const beforeSend = async title => {
  try {
    ...
    if (chatId.value !== null) {
      // 验证现有的kbId是否有效
      if (!kbId.value || kbId.value.trim() === '') {
        console.error('现有对话的知识库ID无效:', kbId.value);
        message.error('当前对话的知识库状态异常，请重新开始对话');
        return;
      }
    ...
    if (!res || !res.kb_id || res.kb_id.trim() === '') {
      console.error('创建知识库失败，返回的ID无效:', res);
      throw new Error('创建知识库失败，服务器返回的知识库ID无效');
    }
    
    kbId.value = res.kb_id;
    console.log('成功创建知识库:', res.kb_id);
    ...
    message.error(e.msg || '创建对话失败，请重试');
    
    // 重置状态
    kbId.value = '';
    chatId.value = null;
    
    throw e; // 重新抛出错误，阻止后续操作
  }
};
