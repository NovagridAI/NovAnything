...
export const useKnowledgeBase = defineStore(
  ...
  () => {
    ...
    const setCurrentId = (id: string) => {
      ...
      if (trimmedId === '') {
        console.warn('知识库ID不能为空字符串');
        currentId.value = '';
        return;
      }
      
      // 验证ID格式（知识库ID通常以KB开头）
      if (trimmedId && !trimmedId.startsWith('KB')) {
        console.warn('知识库ID格式可能不正确:', trimmedId);
      }
      
      currentId.value = trimmedId;
      console.log('设置当前知识库ID:', trimmedId);
    };
