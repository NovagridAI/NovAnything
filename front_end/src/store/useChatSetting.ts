...
export const useChatSetting = defineStore(
  ...
  () => {
    ...
    const setAllChatSettingConfigured = (settings: IChatSetting[]) => {
      ...
      console.log('setAllChatSettingConfigured 输入参数:', settings);
      console.log('当前 activeChatSetting:', activeChatSetting.value);
      
      // 保留至少一个active为true的配置
      if (activeChatSetting.value) {
        const targetModel = settings.find(item => item.serviceId === activeChatSetting.value);
        if (targetModel) {
          targetModel.active = true;
          console.log('根据 activeChatSetting 设置活跃模型:', targetModel.modelName);
        } else {
          console.log('未找到匹配的 serviceId，检查是否有其他活跃模型');
          const hasActive = settings.some(item => item.active);
          if (!hasActive && settings.length > 0) {
            settings[0].active = true;
            console.log('设置第一个模型为活跃:', settings[0].modelName);
          }
        }
      } else {
        const hasActive = settings.some(item => item.active);
        console.log('没有 activeChatSetting，检查是否有活跃模型:', hasActive);
        if (!hasActive && settings.length > 0) {
          settings[0].active = true;
          console.log('设置第一个模型为活跃:', settings[0].modelName);
        }
      ...
      console.log('最终设置的模型配置:', settings);
    ...
    const chatSettingFormActive = computed<IChatSetting>(() => {
      ...
      console.log('chatSettingFormActive computed 被调用');
      console.log('当前 chatSettingConfigured:', chatSettingConfigured.value);
      console.log('找到的活跃模型:', activeModel);
      return activeModel;
    });
