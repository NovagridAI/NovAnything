<template>
  <div v-if="tempDetail && tempDetail.length > 0" class="temp-file-container">
    <div v-for="(item, index) in tempDetail" :key="index" class="temp-file-item">
      <div
        v-if="item.status !== 'green' && item.status !== 'red'"
        class="temp-file-loading-container"
      >
      ...
      <div class="temp-file-item-name" :class="getStatusClass(item.status)">
        {{ item.fileIdName }}
      </div>
