<template>
  <div class="conversation-history">
    <div class="history-header">
      ...
      <arco-button size="small" type="outline" @click="$emit('new-chat')" class="new-chat-btn">
...
import { IconStar, IconStarFill, IconDelete, IconMessage, IconPlus } from '@arco-design/web-vue/es/icon';
  ...
  try {
    ...
    if (response && response.code === 200 && response.data) {
      ...
      if (response.data.qa_logs.length > 0) {
        // 只有在没有当前对话或当前对话不存在时才自动选择对话
        if (!chatId.value) {
          selectConversation(response.data.qa_logs[0]);
        } else {
          // 如果当前对话存在，只更新选中状态，不重新加载对话内容
          const selectedItem = response.data.qa_logs.find(item => item.qa_id === chatId.value);
          if (selectedItem) {
            selectedConversation.value = selectedItem.qa_id;
            // 更新知识库选择，但不清空QA_List
            currentKbid.value = selectedItem?.kb_ids;
            const currentTempId = selectedItem?.kb_ids?.find(item => tempList.value.some(temp => temp.kb_id === item));
            if (currentTempId) {
              setTempId(currentTempId);
            } else {
              setTempId('');
            ...
            setSelectList([...selectedItem?.kb_ids ?? []]);
          } else {
            // 如果当前对话不存在于列表中，选择第一个
            selectConversation(response.data.qa_logs[0]);
          }
