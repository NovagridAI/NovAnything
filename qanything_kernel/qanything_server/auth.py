        ...
        async def decorated_function(req, *args, **kwargs):
            ...
            try:
                ...
                if check_kb_access:
                    ...
                    # 为了确保权限检查的准确性，从数据库实时获取用户角色
                    current_user_role = get_user_role(user_id, req)
                    if current_user_role is None:
                        debug_logger.error(f"用户 {user_id} 不存在或已禁用")
                ...
                # 针对特定错误添加更详细的日志
                if "Python type list cannot be converted" in error_message:
                    debug_logger.error(f"数据类型转换错误 - 用户: {user_id}, 可能是参数类型不匹配")
                    debug_logger.error(f"完整错误信息: {error_message}")
                    return response.json({"code": 500, "msg": "参数类型错误，请检查请求参数"})
