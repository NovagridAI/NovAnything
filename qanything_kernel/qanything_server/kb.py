...
async def new_knowledge_base(req: request):
    ...
    if kb_type == 'team':
        ...
        if user_role not in ['admin', 'superadmin']:
            return sanic_json({
                "code": 403, 
                "msg": "您没有创建团队知识库的权限，只有管理员和超级管理员可以创建团队知识库"
            })
    ...
    try:
        ...
        kb_id, status = local_doc_qa.milvus_summary.kb_dao.new_knowledge_base(kb_id, user_id, kb_name, kb_type)
    ...
    try:
        ...
        for kb in kb_infos:
            kb_dict = {
                ...
                "is_owner": kb[3] == user_id if len(kb) > 3 else True  # 判断是否为所有者
            }
            data.append(kb_dict)
...
async def delete_knowledge_base(req: request):
    ...
    try:
        ...
        for kb_info in kb_info_list:
            ...
            # 如果不是拥有者，检查是否有删除权限
            if not is_owner:
                # 检查是否有admin权限（需要kb_dao中有检查权限的方法）
...
async def get_kb_permission_data(req: request):
    ...
    try:
        ...
        # 获取数据和构建结构
        kb_data = await _get_kb_permission_detailed_data(local_doc_qa, kb_id)
...
async def _get_kb_permission_detailed_data(local_doc_qa, kb_id):
    """获取知识库权限详细数据（辅助函数）
