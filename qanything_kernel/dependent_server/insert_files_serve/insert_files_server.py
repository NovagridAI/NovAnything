...
def get_file_content_hash(file_path):
    ...
    try:
        ...
        insert_logger.error(f"计算文件哈希失败: {file_path}, {e}")
        return None

def acquire_file_lock(file_hash):
    """获取文件处理锁"""
    with lock_dict_mutex:
        if file_hash not in file_processing_locks:
            file_processing_locks[file_hash] = threading.Lock()
        lock = file_processing_locks[file_hash]
    
    # 尝试获取锁，最多等待30秒
    acquired = lock.acquire(timeout=30)
    if acquired:
        insert_logger.info(f"成功获取文件处理锁: {file_hash}")
        return lock
    else:
        insert_logger.warning(f"获取文件处理锁超时: {file_hash}")
        return None

def release_file_lock(lock, file_hash):
    """释放文件处理锁"""
    try:
        lock.release()
        insert_logger.info(f"释放文件处理锁: {file_hash}")
        
        # 清理空锁对象
        with lock_dict_mutex:
            if file_hash in file_processing_locks:
                try:
                    # 尝试再次获取锁，如果成功则说明没有其他线程在使用
                    if file_processing_locks[file_hash].acquire(blocking=False):
                        file_processing_locks[file_hash].release()
                        del file_processing_locks[file_hash]
                        insert_logger.debug(f"清理文件处理锁: {file_hash}")
                except:
                    pass
    except Exception as e:
        insert_logger.error(f"释放文件处理锁失败: {file_hash}, {e}")
...
async def process_data(retriever, milvus_kb, mysql_client, file_info, time_record):
    ...
    # 计算文件内容哈希，用于防止相同文件同时处理
    file_hash = get_file_content_hash(file_location)
    if not file_hash:
        status = 'red'
        msg = f"{file_name} 无法计算文件哈希值"
        return status, content_length, 0, msg
    
    # 获取文件处理锁
    file_lock = acquire_file_lock(file_hash)
    if not file_lock:
        status = 'red'
        msg = f"{file_name} 获取文件处理锁超时，可能有相同文件正在处理中"
        return status, content_length, 0, msg
    ...
    try:
        ...
        # 释放文件处理锁
        release_file_lock(file_lock, file_hash)
