...
from qanything_kernel.configs.model_config import (KB_SUFFIX, UPLOAD_ROOT_PATH, LOCAL_EMBED_PATH, LOCAL_RERANK_PATH, LOAD_LOCAL_TOKENIZERS)
...
def format_source_documents(ori_source_documents):
    source_documents = []
    seen_files = set()  # 用于跟踪已经添加的文件
    
    for inum, doc in enumerate(ori_source_documents):
        file_key = (doc.metadata.get('file_id', ''), doc.metadata.get('file_name', ''))
        
        # 如果这个文件已经添加过了，跳过
        if file_key in seen_files:
            debug_logger.info(f"跳过重复文件: file_id={file_key[0]}, file_name={file_key[1]}")
            continue
        ...
        debug_logger.info(f"添加文件: file_id={file_key[0]}, file_name={file_key[1]}")
    
    debug_logger.info(f"去重前文档数: {len(ori_source_documents)}, 去重后文档数: {len(source_documents)}")
...
def replace_image_references(text, file_id):
    ...
    for line in lines:
        ...
        if match_with_caption:
            ...
            result.append(f"![figure](/novanything/assets/file_images/{file_id}/{image_path}?t={timestamp})")
