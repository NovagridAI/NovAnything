...
class FileDAO(BaseDAO):
    ...
    def check_file_exist(self, user_id: str, kb_id: str, file_ids: List[str]) -> List[Tuple]:
        ...
        # 检查用户角色
        role_query = "SELECT role FROM User WHERE user_id = %s AND status = 'active'"
        role_result = self.execute_query(role_query, (user_id,), fetch=True)
        user_role = role_result[0][0] if role_result else 'user'
        
        if user_role in ['admin', 'superadmin']:
            # 管理员和超级管理员可以操作所有文件（不限制知识库拥有者）
            query = f"""
                ...
                AND kb_id = %s
            """
            result = self.execute_query(query, tuple(file_ids) + (kb_id,), fetch=True)
        else:
            # 普通用户只能操作自己拥有的知识库中的文件
            query = f"""
                SELECT file_id, status FROM File
                WHERE deleted = 0
                AND file_id IN ({file_ids_str})
        ...
        for i in range(0, len(file_names), batch_size):
            ...
            if user_role in ['admin', 'superadmin']:
                ...
                query = f"""
                    ...
                    AND kb_id = %s
                """
                query_params = tuple(batch_file_names) + (kb_id,)
            else:
                # 普通用户只能操作自己拥有的知识库中的文件
                query = f"""
                    SELECT file_id, file_name, file_size, status FROM File
