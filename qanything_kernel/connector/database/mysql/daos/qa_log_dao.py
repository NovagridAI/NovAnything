...
class QaLogDAO(BaseDAO):
    ...
    def get_qa_logs(self, user_id: Optional[str] = None, limit: Optional[int] = None, 
        ...
        if need_info is None:
            ...
            try:
                ...
                if columns_result and len(columns_result) > 0:
                    ...
                    # 如果无法获取列名，则使用预定义的列表
                    debug_logger.warning("无法获取QaLogs表的列名，使用预定义列表")
                    need_info = ["id", "qa_id", "user_id", "bot_id", "kb_ids", "query", "model", 
                                "product_source", "time_record", "history", "condense_question", 
                                "prompt", "result", "retrieval_documents", "source_documents", 
                                "is_favorite", "timestamp"]
            except Exception as e:
                debug_logger.error(f"获取QaLogs表列名失败: {str(e)}，使用预定义列表")
                need_info = ["id", "qa_id", "user_id", "bot_id", "kb_ids", "query", "model", 
                            "product_source", "time_record", "history", "condense_question", 
                            "prompt", "result", "retrieval_documents", "source_documents", 
                            "is_favorite", "timestamp"]
        ...
        try:
            ...
            # 防止查询失败返回None导致的错误
            if results is None:
                debug_logger.error("get_qa_logs查询返回None")
                return []
                
        except Exception as e:
            debug_logger.error(f"执行get_qa_logs查询失败: {str(e)}")
            return []
        ...
        for result in results:
            try:
                ...
                debug_logger.error(f"处理qa_log记录失败: {str(e)}, result: {result}")
                continue
