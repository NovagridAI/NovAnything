...
class KnowledgeBaseDAO(BaseDAO):
    ...
    def get_knowledge_bases(self, user_id: str, kb_type: Optional[str] = None) -> List[Tuple[str, str, str, Optional[str]]]:
        ...
        try:
            ...
            if user_info and len(user_info) > 0:
                user_role = user_info[0][0] if user_info[0][0] else 'user'
                user_dept_id = user_info[0][1] if len(user_info[0]) > 1 else None
            else:
                user_role = 'user'
                user_dept_id = None
                debug_logger.warning(f"用户 {user_id} 不存在或未激活，使用默认角色")
        except Exception as e:
            debug_logger.error(f"获取用户角色和部门信息失败: {str(e)}")
            user_role = 'user'
            user_dept_id = None
        
        # 首先获取用户直接拥有的知识库（所有角色都能看到自己创建的知识库）
        try:
            if kb_type:
                query = (
            ...
            # 防止查询失败返回None导致的错误
            owned_kbs = owned_kbs if owned_kbs is not None else []
        except Exception as e:
            debug_logger.error(f"获取用户拥有的知识库失败: {str(e)}")
            owned_kbs = []
        
        # 如果是超级管理员，额外获取所有团队知识库（但不包括其他人的个人知识库）
        if user_role == 'superadmin':
            try:
                if kb_type == 'team':
                    # 只查询不属于当前用户的团队知识库
                    query = (
                        f"SELECT kb_id, kb_name, kb_type, description FROM KnowledgeBase WHERE deleted = 0 AND kb_type = 'team' AND user_id != %s AND "
                        f"(kb_id LIKE '%{KB_SUFFIX}' OR kb_id LIKE '%{KB_SUFFIX}_FAQ') ORDER BY kb_name")
                    shared_kbs = self.execute_query(query, (user_id,), fetch=True)
                elif kb_type is None:
                    # 获取所有不属于当前用户的团队知识库
                    query = (
                        f"SELECT kb_id, kb_name, kb_type, description FROM KnowledgeBase WHERE deleted = 0 AND kb_type = 'team' AND user_id != %s AND "
                        f"(kb_id LIKE '%{KB_SUFFIX}' OR kb_id LIKE '%{KB_SUFFIX}_FAQ') ORDER BY kb_name")
                    ...
                    # 如果kb_type是'personal'或其他类型，不获取其他人的知识库
                    shared_kbs = []
                
                # 防止查询失败返回None导致的错误
                shared_kbs = shared_kbs if shared_kbs is not None else []
            except Exception as e:
                debug_logger.error(f"超级管理员获取团队知识库失败: {str(e)}")
                shared_kbs = []
            
            return owned_kbs + shared_kbs
        
        # 对于非超级管理员，获取用户通过权限可以访问的知识库
        try:
            if kb_type:
                if kb_type == 'team':
                    # 只查询团队知识库 - 修复后的查询逻辑
                    if user_dept_id:
                        ...
                        query = f"""
                            ...
                            JOIN KnowledgeBaseAccess kba ON kb.kb_id COLLATE utf8mb4_general_ci = kba.kb_id COLLATE utf8mb4_general_ci
                            WHERE kb.deleted = 0 
                            AND kb.kb_type = 'team'
                            AND kb.user_id != %s
                            AND (
                                (kba.subject_id = %s AND kba.subject_type = 'user')
                                OR (kba.subject_id = %s AND kba.subject_type = 'department')
                            )
                            AND (
                        ...
                        """
                        shared_kbs = self.execute_query(query, (user_id, user_id, user_dept_id), fetch=True)
                    else:
                        # 用户没有部门，只查询用户权限
                        query = f"""
                            SELECT DISTINCT kb.kb_id, kb.kb_name, kb.kb_type, kb.description
                            ...
                            AND kba.subject_type = 'user'
                            AND (
                                kb.kb_id LIKE '%{KB_SUFFIX}'
                                OR kb.kb_id LIKE '%{KB_SUFFIX}_FAQ'
                            )
                        """
                        shared_kbs = self.execute_query(query, (user_id, user_id), fetch=True)
                else:
                    # 个人和临时知识库不能共享
                    shared_kbs = []
            else:
                # 查询所有共享的团队知识库 - 修复后的查询逻辑
                if user_dept_id:
                    # 用户有部门，查询用户权限和部门权限
                    query = f"""
                        SELECT DISTINCT kb.kb_id, kb.kb_name, kb.kb_type, kb.description
            ...
            debug_logger.error(f"获取共享知识库失败: {str(e)}")
            shared_kbs = []
    ...
    def delete_knowledge_base(self, user_id: str, kb_ids: List[str]) -> None:
        ...
        Args:
            user_id: 操作用户ID（用于日志记录）
            kb_ids: 要删除的知识库ID列表
        """
        if not kb_ids:
            return
            
        # 标记知识库为已删除（不限制user_id，因为权限检查已在调用方完成）
        kb_ids_str = ','.join(['%s'] * len(kb_ids))
        query = f"UPDATE KnowledgeBase SET deleted = 1 WHERE kb_id IN ({kb_ids_str})"
        self.execute_query(query, tuple(kb_ids), commit=True)
    ...
    def check_kb_access(self, user_id: str, kb_id: str, required_permission: str) -> bool:
        ...
        # 3. 检查用户是否是知识库所有者
        query = "SELECT user_id, kb_type FROM KnowledgeBase WHERE kb_id = %s AND user_id = %s AND deleted = 0"
        kb_info = self.execute_query(query, (kb_id, user_id), fetch=True)
        if kb_info:
            debug_logger.info(f"用户 {user_id} 是知识库 {kb_id} 的所有者")
            return True
        
        # 获取知识库类型
        query = "SELECT kb_type FROM KnowledgeBase WHERE kb_id = %s AND deleted = 0"
        kb_type_result = self.execute_query(query, (kb_id,), fetch=True)
        kb_type = kb_type_result[0][0] if kb_type_result and len(kb_type_result) > 0 else 'personal'
        
        # 4. 对于团队知识库，管理员默认有读写权限
        if kb_type == 'team' and user_role == 'admin':
            # 管理员对团队知识库有完整权限，包括admin权限（可以查看和修改权限设置）
            debug_logger.info(f"管理员 {user_id} 对团队知识库 {kb_id} 拥有 {required_permission} 权限")
            return True
