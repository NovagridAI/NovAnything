...
class LocalDocQA:
    ...
    async def get_knowledge_based_answer(self, model, max_token, kb_ids, query, retriever, custom_prompt, time_record,
        ...
        images_processed = False  # 添加标志防止重复处理图片
        ...
        async for answer_result in custom_llm.generatorAnswer(prompt=prompt, history=chat_history, streaming=streaming):
            ...
            if resp[6:].startswith("[DONE]"):
                ...
                if total_images_number != 0 and not images_processed:  # 如果有图片且未处理，需要处理回答带图的情况
                    ...
                    # 收集所有图片，跨文档去重处理
                    all_images = []
                    processed_image_paths = set()  # 用于全局去重
                    has_debug_output = False  # 用于控制调试输出只显示一次
                    ...
                    for doc in relevant_docs:
                        ...
                        for image in doc['document'].metadata.get('images', []):
                            ...
                            # 提取图片路径用于去重（去掉markdown语法，只保留路径）
                            path_match = re.search(r'!\[.*?\]\((.*?)\)', image_str)
                            if path_match:
                                image_path = path_match.group(1)
                                # 如果这个图片路径还没有处理过，则添加
                                if image_path not in processed_image_paths:
                                    processed_image_paths.add(image_path)
                                    all_images.append(image_str + '\n')
                                    debug_logger.info(f"添加图片: {image_path}")
                                else:
                                    debug_logger.info(f"跳过重复图片: {image_path}")
                    
                    # 构建最终的图片列表，确保只有一个引用图文标题
                    if all_images:
                        # 直接构建结果，不重复添加标题
                        show_images = all_images
                        debug_logger.info(f"最终show_images: {show_images}")
                        debug_logger.info(f"去重前图片总数: {sum(len(doc['document'].metadata.get('images', [])) for doc in relevant_docs)}, 去重后图片数: {len(all_images)}")
                        
                        # 对最终输出进行过滤，确保只有一个"引用图文如下"标题和去重的图片
                        response['show_images'] = ["\n### 引用图文如下：\n"] + show_images
                    else:
                        response['show_images'] = []
                    ...
                    images_processed = True  # 标记图片已处理，防止重复处理
            yield response, history

    def _filter_and_deduplicate_images(self, show_images):
        """
        过滤和去重图片列表，确保只有一个"引用图文如下"标题和唯一的图片
        ...
        debug_logger.info(f"开始过滤图片，原始列表长度: {len(show_images)}")
        debug_logger.info(f"原始图片列表: {show_images}")
        ...
        for item in show_images:
            # 统计标题数量
            if "引用图文如下" in item:
                title_count += 1
                debug_logger.info(f"发现标题: {item.strip()}")
                continue
                
            # 提取图片路径进行去重
            path_match = re.search(r'!\[.*?\]\((.*?)\)', item)
            if path_match:
                image_path = path_match.group(1)
                # 如果这个图片路径还没有处理过，则添加
                if image_path not in processed_image_paths:
                    processed_image_paths.add(image_path)
                    all_image_content.append(item)
                    debug_logger.info(f"保留图片: {image_path}")
                else:
                    debug_logger.info(f"去重跳过图片: {image_path}")
            else:
                # 如果不是图片格式，也保留
                all_image_content.append(item)
        
        debug_logger.info(f"发现标题数量: {title_count}, 去重后图片数量: {len(all_image_content)}")
        
        # 构建最终结果：一个标题 + 去重后的图片
        if all_image_content:
            filtered_result = ["\n### 引用图文如下：\n"] + all_image_content
            debug_logger.info(f"图片过滤结果: 原始{len(show_images)}项 -> 过滤后{len(filtered_result)}项")
            debug_logger.info(f"最终图片列表: {filtered_result}")
            return filtered_result
        else:
            debug_logger.info("没有图片内容，返回空列表")
            return []
